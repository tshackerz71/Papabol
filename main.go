...
const (
	NmapOutputFile     = "nmap_output.txt"
	WorkingOutputFile  = "working_sni_hosts.txt"
)

// Extract live hosts from nmap_output.txt
func extractLiveHosts(filename string) ([]string, error) {
...
func checkSNI(host string) bool {
	dialer := &net.Dialer{
...
func main() {
	if _, err := os.Stat(NmapOutputFile); os.IsNotExist(err) {
		fmt.Println("\u274C Nmap output file not found!")
		return
	}
